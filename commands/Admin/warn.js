...
module.exports = {
    ...
    async execute (interaction) {
        ...
        if (!member.permissions.has(PermissionFlagsBits.ModerateMembers)) { return await interaction.reply({ content: `You **do not** have the required permissions to use this command!`, ephemeral: true });}
...
async function addWarn(interaction, targetUserId, reason) {
    ...
    .setAuthor({ name: `Warn Command | Developed by Rune`})
    .setColor("#1b1432") 
    .setTitle(`<:staff:1363806434940420127>  warn tool ⤵`)
    .addFields(
...
async function editWarn(interaction, targetUserId, warnId, newReason) {
    ...
    const editEmbed = new EmbedBuilder().setColor("#1b1432"); 

    if (!warningData) {
        editEmbed.setAuthor({ name: `Warn Command | Developed by Rune`})
        .setTitle(`<:staff:1363806434940420127>  warn tool ⤵`)
...
async function removeWarn(interaction, targetUserId, warnId) {
    const warningData = await warningSchema.findOne({ GuildID: interaction.guild.id, UserID: targetUserId });
    const removeEmbed = new EmbedBuilder().setColor("#1b1432");

    if (!warningData) {
        removeEmbed.setAuthor({ name: `Warn Command | Developed by Rune`})
        .setTitle(`<:staff:1363806434940420127>  warn tool ⤵`)
...
async function listWarns(interaction, targetUserId) {
    ...
    const listEmbed = new EmbedBuilder().setColor("#1b1432");

    if (!warningData || !warningData.Content.length) {
        listEmbed.setAuthor({ name: `Warn Command | Developed by Rune`})
        .setTitle(`<:staff:1363806434940420127>  warn tool ⤵`)
        .setDescription(`User with ID ${targetUserId} has **no** warnings.`)
        .setFooter({ text: `Edit warning failed`})
        .setTimestamp();
...
async function getWarnInfo(interaction, targetUserId, warnId) {
    try {
        ...
        const infoEmbed = new EmbedBuilder().setColor("#1b1432");

        if (!warningData) {
            infoEmbed.setAuthor({ name: `Warn Command | Developed by Rune`})
            .setTitle(`<:staff:1363806434940420127>  warn tool ⤵`)
            .setDescription(`User with ID ${targetUserId} has **no** warnings.`)
            .setFooter({ text: `Edit warning failed`})
            .setTimestamp();;
